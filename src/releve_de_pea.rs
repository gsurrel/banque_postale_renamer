...
pub fn parse(lines: std::str::Lines) -> Result<String> {
    ...
    // Amount
    let mut lines = lines.skip_while(|line| !line.trim().starts_with("EVALUATION DE VOS ACTIFS :"));
    if let Some(line) = lines.next() {
        let pattern = "EVALUATION DE VOS ACTIFS : ";
        if let Some(pos) = line.find(pattern) {
    ...
    // Generate title
    Ok(format!("{} Relevé de PEA (+ {})", date?, acc_amount?))
}
